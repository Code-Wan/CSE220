...
public class DancePairs {
    public static void dance_pair(Stack stk) {
        ...
        int count = 1;
        while (!stk.isEmpty()) {
            Dancer temp = (Dancer) stk.pop();
            Dancer temp2 = (Dancer) stk.peek();
            if (temp.getGender() == 'M' && temp2.getGender() == 'F') {
                System.out.println("# " + count++ + " " + temp + " & " + temp2);
            }
            if (temp.getGender() == 'F' && temp2.getGender() == 'M') {
                System.out.println("# " + count++ + " " + temp + " & " + temp2);
            } else {
                if (temp.getGender() == 'M') {
                    maleQueue.enqueue(temp);
                } else {
                    femaleQueue.enqueue(temp);
                }
            }
        }

        while (!maleQueue.isEmpty() && !femaleQueue.isEmpty()) {
            Dancer male = (Dancer) maleQueue.dequeue();
            Dancer female = (Dancer) femaleQueue.dequeue();
            System.out.println("#" + count++ + ": " + male + " & " + female);
        }

        if (!femaleQueue.isEmpty()) {
            System.out.print("Unpaired Female: ");
        ...
        if (!maleQueue.isEmpty()) {
            System.out.print("Unpaired Male: ");
